...
const float freqNoteTable[] = {
		        ...
		        2793.82592773,  2959.95532227,  3135.96337891};
...
float getOscSample(Osc* osc){
	return (int32_t)((osc->phase<<1)-osc->period)*(osc->gain/osc->period);
}
...
void applyMods(Osc* osc){
	...
	uint8_t index=NUM_FREQ_MOD_PORTS;
	while(index--){
